...
def create_shield_3d(position: Vec3, rarity: str) -> Entity:
    ...
    if rarity == c.RARITY_COMMON:
        face_color = ursina_color.rgb(120, 90, 60)  # Wooden brown
        rim_color = ursina_color.rgb(80, 60, 40)  # Dark brown
        boss_color = ursina_color.rgb(140, 140, 140)  # Iron
        has_glow = False
    elif rarity == c.RARITY_UNCOMMON:
        face_color = ursina_color.rgb(140, 140, 150)  # Steel gray
        rim_color = ursina_color.rgb(100, 100, 110)  # Dark steel
        boss_color = ursina_color.rgb(180, 140, 80)  # Brass
        has_glow = False
    elif rarity == c.RARITY_RARE:
        face_color = ursina_color.rgb(150, 180, 255)  # Blue steel
        rim_color = ursina_color.rgb(100, 130, 200)  # Dark blue
        boss_color = ursina_color.rgb(192, 192, 192)  # Silver
        has_glow = False
    elif rarity == c.RARITY_EPIC:
        face_color = ursina_color.rgb(200, 100, 255)  # Purple
        rim_color = ursina_color.rgb(150, 50, 200)  # Dark purple
        boss_color = ursina_color.rgb(220, 180, 100)  # Gold
        has_glow = True
        glow_color = ursina_color.rgb(200, 100, 255)
    else:  # LEGENDARY
        face_color = ursina_color.rgb(255, 215, 0)  # Gold
        rim_color = ursina_color.rgb(200, 160, 0)  # Dark gold
        boss_color = ursina_color.rgb(100, 200, 255)  # Bright cyan
        has_glow = True
