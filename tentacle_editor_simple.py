"""
Tentacle Editor 2.0 - Phase 1 Complete

Enhanced tentacle creature editor with real-time parameter controls.

Three mathematical algorithms for generating perfect tentacles:
1. Catenary Curves - Physics-based hanging chain formula
2. Bezier Curves - Cubic polynomial curves with control points
3. Fourier Series - Wave composition for organic shapes
...
def catenary_curve(anchor, target, num_points, sag_factor=0.5):
    """
    Generate points along a catenary curve (hanging chain).

    The catenary is the curve formed by a hanging chain under gravity.
    Formula: y = a * cosh(x/a) where a controls the sag.

    Args:
        anchor: Vec3 start point (on body)
        target: Vec3 end point (tentacle tip)
        num_points: Number of points to generate
        sag_factor: How much the curve sags (0.1=tight, 1.0=loose)
    ...
    # Calculate curve in 2D, then rotate to match anchor->target direction
    direction = (target - anchor).normalized()
    length = (target - anchor).length()

    # Catenary parameter (controls sag)
    a = length / (2.0 * sag_factor)

    # Generate points along catenary in local space
    for i in range(num_points):
        t = i / max(1, num_points - 1)

        # X coordinate along curve
        x = (t - 0.5) * length

        # Y coordinate: catenary formula
        # y = a * cosh(x/a) - a (shifted so it passes through endpoints)
        y = a * (math.cosh(x / a) - 1)

        # Apply sag (negative y means downward)
        y = -y * sag_factor

        # Z is always 0 in local space
        local_point = Vec3(x, y, 0)

        # Transform to world space
        # Create rotation to align with direction
        up = Vec3(0, 1, 0)
        if abs(direction.y) > 0.99:  # Nearly vertical
            up = Vec3(1, 0, 0)
        ...
        world_x = anchor + direction * (t * length) + right * local_point.x + up * local_point.y
        points.append(world_x)
...
class TentacleEditor:
    ...
    def __init__(self):
        ...
        print("\n" + "=" * 70)
        print("TENTACLE EDITOR 2.0 - PHASE 1 COMPLETE")
        print("=" * 70)
        print("NEW FEATURES:")
        print("  - Real-time parameter sliders for all algorithms")
        print("  - Thickness & taper controls")
        print("  - Preset system (4 presets)")
        print("  - Undo/Redo (Ctrl+Z / Ctrl+Y)")
        print("  - Help overlay (Press H)")
        print("")
        print("Quick Controls:")
        print("  H - Help  |  1/2/3 - Tentacles  |  Q/W/E - Algorithm")
    ...
    def create_ui(self):
        ...
        self.ui_elements.append(self.catenary_sag_label)

        self.catenary_sag_slider = Slider(
            min=0.1, max=1.5, default=0.6, step=0.05,
        ...
        self.ui_elements.append(self.catenary_sag_value)
        ...
        presets = [
            ...
            ('Droopy', 'catenary', {'sag_factor': 1.2}),
    ...
    def create_help_overlay(self):
        ...
        help_text = """TENTACLE EDITOR 2.0 - CONTROLS
...
ALGORITHM:
  Q - Catenary (physics-based)
  W - Bezier (smooth curves)
  E - Fourier (wave composition)
  Sliders - Adjust parameters
...
OTHER:
  ...
  ESC - Quit"""
    ...
    def update_ui(self):
        ...
        self.btn_catenary.color = color.rgb(0.5, 0.7, 0.5) if self.algorithm == 'catenary' else color.rgb(0.3, 0.3, 0.4)
        ...
        is_catenary = self.algorithm == 'catenary'
        is_bezier = self.algorithm == 'bezier'
        is_fourier = self.algorithm == 'fourier'

        self.catenary_sag_label.visible = is_catenary
        self.catenary_sag_slider.visible = is_catenary
        self.catenary_sag_value.visible = is_catenary
        ...
        if is_catenary:
            val = self.params['catenary']['sag_factor']
            self.catenary_sag_slider.value = val
            self.catenary_sag_value.text = f"{val:.2f}"
        elif is_bezier:
    ...
    def on_param_changed(self):
        ...
        if self.algorithm == 'catenary':
            self.params['catenary']['sag_factor'] = self.catenary_sag_slider.value
            self.catenary_sag_value.text = f"{self.catenary_sag_slider.value:.2f}"
...
def main():
    """Main entry point"""
    app = Ursina(
        title="Tentacle Editor 2.0 - Phase 1",
        borderless=False,
        fullscreen=False
    )
