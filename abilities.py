    ...
    def use(self, user, target_pos: Tuple[int, int], game) -> Tuple[bool, str]:
        ...
        for enemy in game.enemies[:]:
            ...
            if dist_x <= self.radius and dist_y <= self.radius:
                ...
                game.anim_manager.add_particle_burst(enemy.x, enemy.y, QColor(255, 100, 0), count=10, particle_type="square")

                if enemy.hp <= 0:
                    game.enemies.remove(enemy)
                    xp = enemy.xp_reward
                    user.gain_xp(xp)
                    game.anim_manager.add_blood_splatter(enemy.x, enemy.y)
...
class Whirlwind(Ability):
    ...
    def use(self, user, target_pos: Tuple[int, int], game) -> Tuple[bool, str]:
        ...
        if hit_count > 0:
            game.anim_manager.add_screen_shake(3.0, 0.15)
...
class ShadowStep(Ability):
    ...
    def use(self, user, target_pos: Tuple[int, int], game) -> Tuple[bool, str]:
        ...
        game.anim_manager.add_particle_burst(behind_x, behind_y, QColor(100, 0, 150), count=15, particle_type="star")

        if target_enemy.hp <= 0:
            game.enemies.remove(target_enemy)
            xp = target_enemy.xp_reward
            user.gain_xp(xp)
            game.anim_manager.add_blood_splatter(target_enemy.x, target_enemy.y)
